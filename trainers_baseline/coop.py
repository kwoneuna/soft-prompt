...
class CoOp(TrainerX):
    ...
    def _log_prompt_stats(self):
            ...
            epoch = self.epoch
            with torch.no_grad():
                # ctx_vecs = [] # prompt 간 similarity 계산을 안 할 경우 필요 없음

                # K개 prompt learner 순회 대신 단일 객체에 직접 접근
                # self.model.prompt_learner가 단일 PromptLearner 객체라고 가정
                pl = self.model.prompt_learner # 단일 프롬프트 학습기 객체
                k = 0 # 프롬프트가 하나이므로 인덱스 0 사용
                ...
                # cosine similarity 계산 로직은 단일 프롬프트에서는 의미가 없으므로 제거하거나 주석 처리합니다.
                # 필요하다면 아래 코드를 사용하여 정규화된 벡터를 얻을 수 있습니다.
